...
int	check_map(t_game *zz)
{
	...
	if ((fd = open(zz->map_name, __O_DIRECTORY)) != -1)
		exiting("map name is a directory\n", zz);
	if ((fd = open(zz->map_name, O_RDONLY)) == -1)
		exiting("map name .cub isn't treatable\n", zz);
	str = custom_gnl(fd);
	tmp = 0;
	while (str != NULL)
	{
