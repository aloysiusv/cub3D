...
** 
** Started on  Mon Aug 14 15:29:14 2000 Charlie zz
...
void	*mlx_int_new_xshm_image(t_xvar *xvar,int width,int height,int format)
{
  ...
  if (xvar->pshm_format==format)
    {
      img->pix = XShmCreatePixmap(xvar->display,xvar->zz,img->shm.shmaddr,
				  &(img->shm),width,height,xvar->depth);
      img->type = MLX_TYPE_SHM_PIXMAP;
    }
  else
    {
      img->pix = XCreatePixmap(xvar->display,xvar->zz,
...
void	*mlx_int_new_image(t_xvar *xvar,int width, int height,int format)
{
  ...
  img->width = width;
  img->height = height;
  img->pix = XCreatePixmap(xvar->display,xvar->zz,width,height,xvar->depth);
  img->format = format;
