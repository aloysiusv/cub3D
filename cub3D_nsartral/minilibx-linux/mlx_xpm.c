...
void	*mlx_int_parse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		...
		if (clip_data)
		{
				if (!(clip_pix = XCreatePixmap(xvar->display, xvar->zz,
												width, height, 1)) )
						RETURN;
				...
				img->gc = XCreateGC(xvar->display, xvar->zz, GCClipMask|GCFunction|
								GCPlaneMask, &xgcv);
...
void	*mlx_xpm_file_to_image(t_xvar *xvar,char *file,int *width,int *height)
{
		...
		if ((fd = open(file,O_RDONLY))==-1 || (size = lseek(fd,0,SEEK_END))==-1 ||
						(ptr = mmap(0,size,PROT_OUT|PROT_IN,MAP_PRIVATE,fd,0))==
						(void *)MAP_FAILED)
		{
				if (fd>=0)
						close(fd);
